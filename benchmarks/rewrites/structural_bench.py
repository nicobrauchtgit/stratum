...
def available(flag):
    """True if this base exposes `flag`. Rewrites that live only on unmerged PRs
    (log_sum_exp, constant_folding, consecutive_select/drop) are skipped rather
    than erroring, so the harness runs on any base."""
...
CASES = [
    ...
    ("0/x -> 0 (opt-in)",   "zero_div",           lambda: 0 / st.as_data_op(7),                    False, False),
